...
> **2026-09-16 update:** per explicit user instruction, the geocoding line itself is now
> hardcoded (`place = 'Cocle, Penonome'`, no more `input()`) in all three companion files. The
> token and the rest of the download/parse flow are still off-limits. See §8 row 19.
...
- **SUPERSEDED — HONEST BASELINE is now Ensemble 8.33% MAPE / 110.7 MW RMSE (§12.6)**, after
...
| 6 | Hardcoded Renewables.ninja API token in source | Data-download cell | Briefly moved to an env var (v26.3), then **restored to in-cell by user request** (v26.4) — the user manages the token in the download cell directly. Token is in the repo and its git history | Open — **rotate the token** at renewables.ninja before any public sharing (user action) |
...
| 19 | **Geocoding location hardcoded per explicit user request** — the user no longer wants to type the location in at runtime | `place = input(...)` line, both `.py` files and the notebook's matching cell; README step 2 and the ".py calls input()" sentence, now stale | Replaced with `place = 'Cocle, Penonome'` — the exact string the notebook's own saved cell output shows was typed in the run behind every reported result (resolves to lat=8.5204989, lon=-80.359579, matching §0/§1's rounded lat=8.52, lon=-80.36), so this is not a new/guessed location. Token and the rest of the download/parse flow left untouched — D-09's rule still applies to those | Done per explicit instruction (D-09 update, `decisions.md`); geocoding call itself (`geolocator.geocode(place)`) still runs live, only the interactive prompt is gone |
...
> **All numbers in §9 are SUPERSEDED by §12.6 (2026-07-23).** They were computed with the
...
**3. CRITICAL — solar irradiance calibration leaks target-hour information (NEW finding, not
